...
from api.schemas.nodes import EnvironmentCreate, StatusSnapshotCreate, VMConfigCreate, LXCConfigCreate
...
def test_create_vm_config(db: Session):
    ...
    env = nodes.create_environment(db, EnvironmentCreate(
        vmid=101,
        name="vm-config-test",
        type="qemu",
        node_name="pve1",
    ...
    config = nodes.create_vm_config(db, env.id, "seabios", "pc", "virtio-scsi-pci", "debian.iso", "cdn")
